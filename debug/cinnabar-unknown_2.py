...
from cinnabar.prediction_rejection.PredictionRejection import compute_value, \
    EntropyRejection, SufficientlySafe, ValueAware, SPROUTRejection, SPROUTStrategy, EnsembleRejection
from cinnabar.utils.dataset_utils import read_unknown_tabular_dataset
...
SCORES_FILE = "test_unk_et_xgb.csv"
...
def get_rejection_strategies(cost_matrix, classifier, data_dict: dict) -> list:
    ...
    knn = SPROUTRejection(cost_matrix=cost_matrix, reject_cost=REJECT_COST, x_train=data_dict["x_train"],
    ...
    for rej in rej_list:
        ...
        rejectors.append(EnsembleRejection(
            cost_matrix=cost_matrix, reject_cost=REJECT_COST,
            rejectors=[knn, rej], strategy='one'))
